...
# fidelity in Maharashtra
 ...
 site_fidelity %>% 
  ...
  ggplot(aes(MONTH, PRED, colour = COVID)) +
     labs(title = paste("Site fidelity in", anal_states$a, "state"),
...
urban1 %>% 
  ...
  geom_errorbar(aes(ymin = CI.L, ymax = CI.U),
                size = 1.5, width = 0.2, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette, name = "Migratory year", labels = covid_labels)
...
bang2 %>% 
  ggplot(aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
